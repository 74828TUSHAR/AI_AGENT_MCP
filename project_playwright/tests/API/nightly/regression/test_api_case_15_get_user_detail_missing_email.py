...
@allure.title("API Case 15 - Get user detail without email")
async def test_api_case_15_get_user_detail_missing_email(api_client, test_record):
    scenarios = {
        "scenario_1": "Send GET request to user detail API without email and verify bad request response"
    }
    log_api_header(logger, "test_api_case_15_get_user_detail_missing_email", scenarios)

    test_name = "test_api_case_15_get_user_detail_missing_email"

    # ============================================================
    # SCENARIO 1: Send GET request to user detail API without email and verify bad request response
    # ============================================================
    with allure.step("Scenario 1: Get user detail without email"):
        async def scenario():
