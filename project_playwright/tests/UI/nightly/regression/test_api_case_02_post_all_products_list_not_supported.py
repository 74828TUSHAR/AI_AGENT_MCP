...
@allure.title("API Case 02 - POST to all products list is not supported")
async def test_api_case_02_post_all_products_list_not_supported(api_client, test_record):
    scenarios = {
        "scenario_1": "Send POST request to products list API and verify method not supported response"
    }
    log_api_header(logger, "test_api_case_02_post_all_products_list_not_supported", scenarios)

    test_name = "test_api_case_02_post_all_products_list_not_supported"

    # ============================================================
    # SCENARIO 1: Send POST request to products list API and verify method not supported response
    # ============================================================
    with allure.step("Scenario 1: POST products list"):
        async def scenario():
