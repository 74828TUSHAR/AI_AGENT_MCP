...
@allure.story("Manual Scroll Up")
@allure.title("Verify scroll up without arrow button")
async def test_case_26_scroll_up_without_arrow(page, env, test_record):
    scenarios = {
        "scenario_1": "Navigate to homepage and scroll to footer",
        "scenario_2": "Verify subscription section at footer",
        "scenario_3": "Scroll to top manually and verify hero text",
    }
    log_test_header(logger, "test_case_26_scroll_up_without_arrow", scenarios)
    ...
    with allure.step("Scenario 1: Scroll to footer"):
        try:
            ...
            await page.screenshot(path="screenshots/case_26_scenario_1.png")
    ...
    with allure.step("Scenario 2: Verify subscription at footer"):
        try:
            ...
            await page.screenshot(path="screenshots/case_26_scenario_2.png")
            raise AssertionError(f"Scenario 2 failed: {e}") from e

    # ============================================================
    # SCENARIO 3: Scroll to top manually and verify hero text
    # ============================================================
    with allure.step("Scenario 3: Scroll to top manually"):
        try:
            await home_page.scroll_to_top()
            assert await home_page.hero_text_visible()
            logger.info("Scenario 3 passed")
        except Exception as e:
            line_num = get_exception_line()
            logger.error(f"Scenario 3 failed at line {line_num}: {e}")
            await page.screenshot(path="screenshots/case_26_scenario_3.png")
